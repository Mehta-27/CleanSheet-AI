    ...
    if profile.missing_values:
        ...
        with st.expander("❌ Missing Values", expanded=True):
            st.markdown(
                ...
                f"({profile.total_missing_cells / max(profile.total_cells, 1) * 100:.1f}% of all cells)"
            )
    ...
    if profile.duplicate_count > 0:
        ...
        with st.expander("🔁 Duplicate Rows", expanded=True):
            st.markdown(
                ...
                f"({profile.duplicate_count / max(len(df), 1) * 100:.1f}% of data)"
            )
            if profile.duplicate_rows:
                ...
                if len(profile.duplicate_rows) > 10:
                    st.caption(f"Showing 10 of {len(profile.duplicate_rows)} duplicates")
    ...
    if profile.outlier_candidates:
        ...
        with st.expander("📊 Potential Outliers", expanded=False):
            ...
            for col, vals in profile.outlier_candidates.items():
                st.markdown(
                    f"- **{col}**: {len(vals)} outliers detected"
                )
